...
cols={str(i):row1[i] for i in range(0,len(row1))}    #获取行索引
col4=sheet.col_values(3)                                  #取出第4列，
rows={str(i):col4[i] for i in range(0,len(col4))}    #获取列索引
...
rindex=list(rindex)                                        #获取行列表
rindex="".join(rindex)                                    #提取行数据
rindex=int(rindex)                                         #转换数据类型     

cindex=list(cindex)                                        #获取列列表
cindex="".join(cindex)                                    #提取列数据
